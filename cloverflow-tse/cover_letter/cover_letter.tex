...
We started the investigation by using a minimum clone size of six lines, a
well-accepted minimum clone size in clone benchmark~\cite{Bellon2007}. Simian
reported 67,172 clone candidate pairs and SourcererCC reported 7,752 clone
candidate pairs. We randomly sampled 382 pairs from the two sets and the first
author investigated them. This was a statistical significant sample with a 95\%
confidence level and $\pm 5\%$ confidence interval. We found 26 non-clone pairs;
322 trivial clone pairs including \texttt{equals}, \texttt{hashCode}, or
getters/setters; and 34 non-trivial clone pairs. Next, we increased the minimum
clone size to ten lines, another well-established minimum clone size for
...
clone pairs and 102 non-trivial clone pairs. Then, we increased the minimum
clone size further to fifteen lines and retrieved 196 clone pairs from Simian
and 1,230 clone pairs from SourcererCC. The manual check of the 382 randomly
sampled pairs reveal zero non-clone pairs, 298 trivial clone pairs and 83
non-trivial clone pairs.

The findings from the three samples show that selecting the minimum of ten lines
was a sweet spot and was preferred over six and fifteen lines. First, it generated 
