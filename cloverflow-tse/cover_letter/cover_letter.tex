...
%		\caption{Clones of the 100 Stack Overflow outdated code snippets in 131,703 GitHub projects}
	...
	We also updated the Overall Discussion section to reflect the findings that there is only a few toxic outdated code that are found in open source projects on GitHub. %Moreover, the direction of copying is also unclear. The clones could come from Stack Overflow, the original project repositories, or other places. 
\end{enumerate}
...
The research question (section 3.5.3) is actually about notifying the answerers that their answers are already outdated, not notifying the users of the outdated code snippets. This notification to the answerer can be done by posting comments for that answer on Stack Overflow. For example, the answerer of the Stack Overflow question no.~10924648 posted a comment on his own answer saying that the answer is specifically for Spring 3.1. Moroever, another user posted a comment on the answer of the post no.~801935 that the code snippet is not of high quality and suggested an alternative in another post. We extended this discussion in RQ4 \FIXME{do it}. 
