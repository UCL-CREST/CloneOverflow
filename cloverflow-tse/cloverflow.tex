...
we found 214 code snippets on Stack Overflow that violate the license of their
original software. We surveyed 201 high-reputation Stack Overflow answerers
(33\% response rate) and found that 69 of them have never been notified of
outdated code in the answers and 26 of them rarely or never fix the outdated
code. Only 3 answerers explicitly include software license in their answers and
138 answerers never check for licensing conflicts between their copied code
...
Overflow that involves the reuse of source code can be considered
code cloning.
...
public int compare(byte[] b1,int s1,int l1, ...) {    public int compare(byte[] b1,int s1,int l1, ...) {
  ...
  return compare(key1,key2); /* compare them */         }
	...
	\caption{An example of the two code fragments of
		...
		(right). Its Apache v.2.0 license is also missing.~\FIXME{@Jens Could you please make the underline clearer?}}
...
The absence of Apache v.2.0 license also occurs after the snippet is posted on 
Stack Overflow.
...
\begin{enumerate} 
	...
	from a specific open source project and that 101 of them are outdated. %
...
using regular expressions and manual checking. ~\FIXME{@Jens, should I mention I
	use PMD CPD to help filtering SO code snippets here? Will it be too confusing?}.
...
source code. The median size of the snippet is 17 lines.
...
tools' creators as depicted in \Cref{tab:clone_config}. All variable renaming are disabled to decrease the number of
...
copying while the other could not. They result in a better classification,
 i.e.~from UD to QS or EX.~\FIXME{@Jens Is it better to just report \% of conflicts
 between TP vs.~FP classifications (e.g. only from QS, SQ, EX, UD, BP, IN <--> AC)?
Because 26\% does not come from a wrong validation, it's mostly just a different classification.}
...
\begin{table}
	...
	\begin{tabular}{c@{~~}p{7.35cm}}
		...
		EX & Cloned from an external source \textit{X} (X $\rightarrow$ S). \\
...
\cite{Ragkhitwetsagul_RN2017}~\FIXME{Fix the RN bibtex}.
...
The most cloned projects is \textsf{hibernate} with 24
...
intra-clone instances in Qualitas, we obtained 111 EX pairs. We found evidence
of copying from an external source to Stack Overflow, but not in Qualitas, in 61 pairs. 
A stronger evidence of copying from an external source to both Stack
Overflow and Qualitas is available in the other 50 pairs. Each of them contains
statement(s) pointing to the original external location of the cloned code. Our
...
\begin{table}
	...
	\resizebox{\linewidth}{!}{
		\begin{tabular}{lp{2.3cm}p{2.3cm}rrr}
			...
			& CC BY-SA 3.0 & No license & & 1 & \\
			\midrule
			\multicolumn{3}{l}{Total} & 23 & 15 & 3 \\
			\midrule
			Incompat. 
			& No license & AGPLv3/3+ & 1 & & 4 \\
			& No license & Apache-2 & 46 & 14 & 12 \\  
			& No license & BSD/BSD3 & 4 & & 1 \\ 
			& No license & CDDL or GPLv2 & & & 6 \\ 
			& No license & EPLv1 & 10 & & 6 \\
			& No license & GPLv2+/3+ & 8 & 48 & 7 \\ 
			& No license & LesserGPLv2.1+/3+ & 16 & & 9 \\ 			
			& No license & MPLv1.1 & & & 1 \\ 
			& No license & Oracle & & 3 \\ 
			& No license & Proprietary & & 1 & 2 \\ 
			& No license & Sun Microsystems & & 1 & 2 \\ 
			& No license & Unknown & & 11 & \\
			& New BSD3 & LesserGPLv2.1+ & 1 & & \\ 
			\midrule
...
results are shown in \Cref{fig:survey_snippet_source}. Looking at Very
...
The work that are closely similar to us are studies by An et al.~\cite{An2017}
and Abdalkareem et al.~\cite{Abdalkareem2017}. An et al.~investigated clones
...
android apps from F-Droid repository and used timestamp to determine direction
of copying. They found 22 apps containing code cloned from Stack Overflow. They
reported that cloned code are commonly used for enhancing existing code. Their
