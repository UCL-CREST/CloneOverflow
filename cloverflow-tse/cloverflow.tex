...
in the curated Qualitas corpus. We analysed 2,289 non-trivial online clone
...
Nevertheless, we investigated a crucial parameter setting for clone detection: the minimum clone size threshold.
Choosing a large clone size threshold can reduce the number of false clones reported,
while at the same time creates some false negatives, i.e.~missing some of the true clones.
On the other hand, setting a low threshold results in a larger number of clone
candidate pairs to look at and a higher chance of getting false positives.
It is known that lowering the number of minimum clone lines is harmful to precision.
Moreover, the large number of clone pairs will hinders the full manual
validation of the clones and one needs to rely on sampling of clone pairs.
We started our investigation by using a minimum clone
size of 6 lines, a well-accepted minimum clone size in clone
benchmark~\cite{Bellon2007}. Simian reported 67,172 clone candidate pairs and
SourcererCC reported 7,752 clone candidate pairs. We randomly sampled 382 pairs
from the merged set of 74,148 clone pairs and the first author investigated them. This was
a statistical significant sample with a 95\% confidence level and $\pm 5\%$ confidence interval.
We found 322 trivial clone pairs including \texttt{equals}, \texttt{hashCode}, or getters/setters;
26 non clone pairs, and 34 non-trivial clone pairs.
We then increased the minimum clone size to 10 lines, another well-established 
minimum clone size for large-scale data sets~\cite{Sajnani2016}, and retrieved 721 clone pairs from Simian and
1,678 clone pairs from SourcererCC. We randomly sampled and manually checked the same amount of 382 pairs
from the merged set of 2,289 clone pairs.
We found 251 trivial clone pairs, 40 non-clone pairs, and 91 non-trivial clone pairs.

The findings from the two samples show that selecting the minimum of 6 lines generated
a large number of clones (74,148 pairs) with more trivial clone pairs, which we were not interesting for this study, 
and fewer non-trivial pairs compared to the minimum of 10 lines.
%To put it another way, lowering the number of clone lines is harmful to precision.
Moreover, the amount of 2,289 clone pairs using minimum of 10 clone lines 
was feasible for manual investigation in the next phase.
Increasing the minimum clone size further will result in a smaller clone subset
but may also remove some of the true clone pairs (generating false negatives).
Thus, we selected the minimum clone size of 10 lines for both Simian and SourcererCC.
...
projects. We selected SourcererCC with 80\% similarity for this task because it
...
posting comments below the answer on Stack Overflow. For example, the answerer of
...
\vspace{0.25cm}
\textit{How often do Stack Overflow
	visitors experience the outdated code and licensing conflicts when
	they reuse code in an answer from Stack Overflow?}
\vspace{0.25cm}
...
by code deprecation (15\%). There were 10 outdated code snippets (10\%) due to bug fixing.
...
Then, we performed the check for clones of the 100 outdated code snippets in 130,703 GitHub projects.
We found 102 cloned candidates appearing in 68 GitHub projects and manually investigated all
of them. As shown in \Cref{tab:outdated_github}, out of the 102 cloned snippets, there were
...
projects, there were 47 cloned snippets that were exactly the same as the
outdated code snippets and 42 cloned snippets that were not exactly the same
(e.g.~they contained additional code modifications made by the projects'
developers or they were copied from another source with a slightly different
code).
...
in the answers. The other 32 cloned snippets were very likely to be a file-level
clone from its respective original project (e.g. JFreeChart, JUnit, Log4J,
Hadoop) based on their license header and the Javadoc comments. Lastly, the
remaining 13 cloned snippets did not have any hints or evidence of copying.
...
directly. Nevertheless, we decided to report the developers of the two projects
...
outdated version of \texttt{humanReadableInt()} method and proposed a fix by
...
problematic and they decided to implement their own fix due to a concern of a
potential software licensing conflict caused by copying the fix directly from
Hadoop code base. On the other hand, the bug report in Apache Hive is still
unfixed.
...
Stack Overflow has a voting mechanism that may mitigate this issue, the
accepted answer is still used by naive developers who copy and reuse the
outdated code.
Although we did not find strong evidence of the outdated code snippets
being cloned to GitHub projects, it would still be useful if Stack Overflow
implements a flagging of outdated answers.
...
without evidence of copying. Two of them contains a race condition and
...
130,703 GitHub projects using SourcererCC with 80\% similarity threshold. Then,
we used the Ninka tool to automatically identify software license from the 7,207
code snippets. Opposite to the outdated clones, we discovered many more clones
of 7,207 candidate pairs. There were 90 pairs from 10 Qualitas projects on
GitHub and 7,117 pairs from 2,427 other projects. From
\Cref{tab:license_github}, the clones were found from highly-starred
...
We found 7,117 of the 214 license-incompatible code snippets
in 130,703 GitHub projects.
\end{boxquote}
...
we are proposing to the software engineering research and the Stack Overflow community.
...
or the number of minimum lines of code to be consider copying, i.e.~de minimis,
...
copied from Qualitas to Stack Overflow. We call for more research in
...
the origin of code clones. Since automatically establishing code cloning
