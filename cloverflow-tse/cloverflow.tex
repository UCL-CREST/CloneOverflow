...
(33\% response rate) and found that 69 of them (FIXME\%) have never been notified of
outdated code in the answers and 26 of them (FIXME\%) rarely or never fix the outdated
code. Only 3 answerers explicitly include software license in their answers and
138 answerers (FIXME\%) never check for licensing conflicts between their copied code
...
\FIXME{I don't understand the following:} The absence of Apache v.2.0 license also occurs after the snippet is posted on 
Stack Overflow.
...
\FIXME{Something is not right: First it says that variable renaming is
  disabled, and later it is said that SourcererCC uses renaming.}
All variable renaming are disabled to decrease the number of
...
copying while the other could not. They result in a better classification,
 i.e.~from UD to QS or EX.~\FIXME{Report \% of conflicts
 between TP vs.~FP classifications first, and then about evidence.}
...
\begin{table}
	...
	\begin{tabular}{c@{~~}p{7.35cm}}
		...
		UD & Cloned from each other or from an external source
                     \textit{X} outside the project (unknown)\\
		\midrule
...
\begin{table}
	...
	\caption{License mapping of online clones (file-level)
          \FIXME{Change order of columns, Qualitas before SO}}
...
Note that the code snippets could violate the license, but do
...
participants (FIXME\%) answer questions on Stack Overflow every day.
...
Nevertheless, our study might still suffer from false negatives, i.e.~online 
