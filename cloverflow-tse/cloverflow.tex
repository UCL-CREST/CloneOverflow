...
Stack Overflow (e.g.~\cite{Keivanloo2014,An2017,Yang2016}), fewer studies
...
\begin{itemize}
	...
	We surveyed 600 high reputation Stack Overflow users to study their
...
parameterised clones (type-2). \textbf{NiCad} is also a text-based clone
detector which detects clones at either method- or block-level granularity. It
can detect clones of type-1, -2 up to type-3 (clones with added and removed
statements) and has also been used in several empirical clone
studies~\cite{Roy2008, Ragkhitwetsagul2016, Svajlenko2014, Wang2013, Mondal2011,
	Sajnani2016}. NiCad utilises TXL~\cite{Cordy2006} for parsing and
pretty-printing source code and provides code normalisation by variable renaming
and abstraction. We use a variant of NiCad called \textit{nicadcross}. It offers
the same functionalities as the original NiCad but is specialised for detecting
code clones between two systems.
...
data. Simian did not provide an option to detect cross-project clones. Hence the
...
%		\resizebox{\columnwidth}{!}{
	\small
	\begin{tabular}{p{0.8cm}|p{2.5cm}p{3.8cm}}
		\toprule
...
than an \textit{ok} pair. We follow Bellon's original definitions of
good- and ok-match~\cite{Bellon2007}, which are based on how much two
clone fragments \textit{CF} overlap\footnote{The overlap value can be
	considered as Jaccard similarity.}  and how much of them is
...
(\textit{CF$_1$}, \textit{CF$_2$}), and the \textit{good-value} and
the \textit{ok-value} of two clone pairs are defined as
...
clone pairs is 32,316, 0.1\%~of all reported clone
...
\begin{table}
	...
	\begin{tabular}{l|r|r|r|r|r}
		...
		\textit{ok} & 12,063 & 993 & 79 & 20,021 & 32,316 \\
		\bottomrule
	\end{tabular} %
	% }
\end{table}
...
pairs\footnote{This excludes the subsumed 2,313 \textit{good}
	...
	\textit{good} pairs.}. We obtained 12,063; 993; 79; and 20,021 pairs
...
\begin{table}
	...
	\begin{tabular}{l|r|c|c|r}
		\toprule
		\multirow{2}{*}{Set} & \multirow{2}{*}{Before Filtering} & \multicolumn{2}{c|}{Filters*} & \multirow{2}{*}{After Filtering} \\ \cmidrule{3-4}
		& & \textit{L} & \textit{S} & \\ 
		\midrule 
...
\begin{figure}
	\centering
	\includegraphics[width=\linewidth]{classification_process}\vspace{-1ex}
...
from their repositories on September 26, 2016. For each \textbf{QS} online clone
...
method name. We then compared the Stack Overflow snippet to its latest version
line-by-line to find if any change has been made to the source code. We also
made sure that the changes did not come from the modifications made to the Stack
Overflow snippets by the posters but from the updates in the projects
themselves. When we found inconsistent lines between the two versions, we used
{\small\texttt{git blame}} to see who modified those lines of code and the
timestamps.
...
their previous answers. The full survey can be found in our \FIXME{technical
	report/website?} ~\footnote{Stack Overflow Answerers' Survey:
	\url{http://www.cs.ucl.ac.uk/staff/C.Ragkhitwetsagul/files/so_survey.pdf}}.
...
0.1\%~of all pairs are agreed by two tool/configuration
...
\begin{table}
	...
	\begin{tabular}{lrrr}
		\toprule
		\multirow{2}{*}{Set} & \multirow{2}{*}{Candidates} & \multicolumn{2}{c}{Classification} \\ %\cmidrule{3-4}
		& & Auto & Manual \\
		\midrule 
...
\begin{figure}
	\begin{subfigure}{.25\textwidth}
		\centering
		\includegraphics[width=.8\linewidth]{survey_exp_1-crop}
		\caption{Group 1}
		\label{fig:survey_group1_exp}
	\end{subfigure}%
	\begin{subfigure}{.25\textwidth}
		\centering
		\includegraphics[width=.82\linewidth]{survey_exp_2-crop}
		\caption{Group 2}
...
\begin{figure*}
	\begin{subfigure}{.5\textwidth}
		\centering
		\includegraphics[width=\linewidth]{survey_snippet_source_1}
		\caption{Group 1}
		\label{fig:survey_snippet_source_1}
	\end{subfigure}%
	\begin{subfigure}{.5\textwidth}
		\centering
		\includegraphics[width=\linewidth]{survey_snippet_source_2}
		\caption{Group 2}
...
\begin{figure}
	\begin{subfigure}{.25\textwidth}
		\centering
		\includegraphics[width=.8\linewidth]{survey_license_1}
		\caption{Group 1}
		\label{fig:survey_license_1}
	\end{subfigure}%
	\begin{subfigure}{.25\textwidth}
		\centering
		\includegraphics[width=.8\linewidth]{survey_license_2}
		\caption{Group 2}
		\label{fig:survey_license_2}
	\end{subfigure}
	\caption{Do you usually include a software license in your code snippets on Stack Overflow?}
...
\Cref{fig:survey_license}) due to several reasons. First, some answerers posted
only their own code or code that was adapted from the question, hence
...
to claim any intellectual property on them and fall under fair use.
...
\begin{figure}
	...
	\end{subfigure}
	\caption{How frequently did you check the software license of the code snippets you copy to Stack Overflow if they conflict with Stack Overflow's CC BY-SA 3.0 license?}
	\label{fig:survey_license_check}
\end{figure}

We asked the answerers how frequently they checked for a conflict between software license of the code snippets they copied to Stack Overflow and Stack Overflow's CC BY-SA 3.0. As shown in \Cref{fig:survey_license_check}, approximately sixty nine percent of answerers from both groups did not perform the checking. Nonetheless, they are about 10\% of the answerers who frequently checks for licensing conflicts when they copy code snippets to Stack Overflow.
