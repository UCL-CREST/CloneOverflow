...
~\FIXME{should I mention I use PMD CPD to help here? More confusing?}.
Finally we obtained
...
\begin{table}
	...
	\begin{tabular}{lrrr}
		...
		\midrule
		Stack Overflow & 72,365 & 1,840,581 & 10 \\ 
...
while PMD CPD failed to complete the execution. Although Simian, SourcererCC,
CCFinder, iClones, and DECKARD could successfully report clones, we decided to
choose Simian and SourcererCC due to their fast detection speed.
...
corpus~\cite{Sajnani2016,Saini2016,Yang2017}. Due to incomplete code blocks and
functions normally found in Stack Overflow snippets, the built-in Java tokeniser
could not parse more than half of all snippets. SourcererCC provides an option
to plug in a customised tokeniser, so we developed a special Java tokeniser with
assistance from the tool's creator. Another benefit of SourcererCC is its
indexing and search processing allows us to detect code clones between two
systems.
...
\begin{table}
	...
	\begin{tabular}{lrr}
		\toprule
		Stats & \multicolumn{1}{c}{Simian} & \multicolumn{1}{c}{SourcererCC} \\
		\midrule
		Total clone pairs & 721 & 1,678 \\
		Average clone size & 16.61 & 17.86 \\
		\bottomrule
	\end{tabular} %
	%}
\end{table}
...
their previous answers. The full survey can be found in our \FIXME{technical
	report/website?} ~\footnote{Survey--Stack Overflow Answerers:
...
\begin{table}
	...
	\begin{tabular}{rrrr}
		\toprule
		Pairs & Snippets & Projects & Avg.~cloned ratio \\
		\midrule
		2,304 & 460 & 77 & ??\% \\ % <-- for avg. cloned ratio, use scripts in ~/Downloads/stackoverflow/scripts/checking
		\bottomrule
	\end{tabular}
\end{table}
