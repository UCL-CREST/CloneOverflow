...
Figure \ref{fig:1} shows a clone pairs that is reported by only Simian$_{df}$.

\begin{figure}[H]
\centering
\includegraphics[width=1.0\linewidth]{images/1}
\caption{A clone pair solely reported by Simian$_{df}$}
\label{fig:1}
\end{figure}

Figure \ref{fig:2} shows a clone pair that is reported by Simian$_{df}$, NiCad$_{df}$, NiCad$_{fse13}$.

\begin{figure}[H]
\centering
\includegraphics[angle=90,origin=c,width=0.6\linewidth]{images/2}
\caption{A clone pair that is reported by Simian$_{df}$, NiCad$_{df}$, NiCad$_{fse13}$.}
\label{fig:2}
\end{figure}

\section*{More Manual Investigations}
I have chosen the biggest unique 25 clone pairs from the 4 different settings. If I manually investigate them, we will get 100 more.
