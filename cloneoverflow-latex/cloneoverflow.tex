...
projects. We designed our study in 6 phases as depicted in \Cref{fig:exp_framework} where
we build different data sets to answer each of our four research questions. 
We explain our process of building the data sets in each phase separately below.
...
and left 3,636 clone pairs for manual investigation in phase 3 to 6.

\subsubsection{Phase 3-4: Clone Validation and Classification}
We relied on the 3,636 filtered clone pairs after automatic classification for
manual validation and classification.
The validation and classification of the clone pairs were done at the same time. 
The clone validation process involves checking 
if a clone pair is either true positive or false positive. 
...
looked at the clones, and decided if they are true positive or
false positive and classified them into an appropriate pattern. 
%the most appropriate cloning pattern for the pair. 
To mitigate the human errors of the manual investigation, 
...
We have tried to classify 697 online clone pairs from
...
using Kapser's cloning patterns. We found that the
clone pairs were categorised into either Customisation or Templating.
Thus, Kapser's cloning patterns are too broad for our study and a
...
separately for the \textbf{QS}, \textbf{EX}, and \textbf{UD} set.
...
them, the data set still contains 1,216 true positive clone pairs between 516 Stack
...
(\Cref{fig:classification_process}).  
%
