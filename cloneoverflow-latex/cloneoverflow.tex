...
\begin{table}
\caption{FIXME Maybe merge with Table 7? <-- You mean Table 9?}
\label{tab:snippets}
\centering
...
combinations. As shown in FIXME, for the 2,308 \textit{good} pairs,
there are 92 Stack Overflow snippets which are associated with 66
Qualitas projects. The 11,749 \textit{ok'} pairs are clones between
136 Stack Overflow snippets and 39 Qualitas projects.  Moreover, for
the disagreed pairs, we found 17,081 clone pairs reported by $S_D'$
are clones between 471 Stack Overflow snippets and 76 Qualitas
projects and 88 $N_D'$ clone pairs are clones between 77 Stack
Overflow snippets and 29 Qualitas projects. In total we found that the
manually investigated 31,946 clones pairs are clones between 621
Stack Overflow snippets and 95 Qualitas projects.

\textbf{To answer RQ1, with the four configurations of Simian and
  NiCad, we found most Qualitas projects to share similar code with
  Stack Overflow code snippets. When onlly considering the manually
  investigated snippets, FIXME}
...
\textbf{To answer RQ2, we found that a large amount of the clone pairs
  ...
  (28,642) or accidental clones (2,529) \FIXME{IN+AC here?} with no evidence that the code
