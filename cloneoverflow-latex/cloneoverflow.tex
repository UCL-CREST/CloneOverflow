...
\begin{table}
	...
	\begin{tabular}{l|r|r|r|r}
		...
		\multirow{1}{*}{$N_D$} & 168 & 41 & 127 & 116 \\ 
		%\multirow{1}{*}{\textit{disagreed clone pairs}}    & 1,039 \\
		\hline
		Total  & 28,832 & 26,102 & 3,450 & 10,132 \\ 
		\hline
	\end{tabular} %
	%}
\end{table}
...
%\FIXME{Reported the errors to NiCad creator. Cordy said it might be a long-path problem. Should we try fixing?}  
$N_D$ could not detect clones in Hibernate due to clustering errors. $N_E$ generated renaming errors for 5 projects including Vuze, Hibernate, Myfaces, Netbeans, and Spring. We had contacted the creator of NiCad regarding the issues. They identified them as problems of long file paths and TXL grammars which will be fixed in the next NiCad releases. As a result, we did not have NiCad clones from these project in the agreed clone pairs. Simian clones of the same projects also disappeared from the agreed clone pairs since they could not find matches.
...
\begin{table}
	...
	\begin{tabular}{l|r|r|r|r|r|r}
		...
		\textit{ok} & 9,049 & 993 & 79 & 19,777 & 29,898 & 29,058 \\
		\hline
	\end{tabular} %
}
\end{table}
...
In our manual investigation process, we used a script to automatically classify \texttt{equals()} clone pairs which helped classifying 26,102 pairs out of 47,068 (more than half of the \texttt{equals()} pairs came from $S_D$). Finally, there are 3,450 pairs remaining which we manually investigated them. \textbf{To answer RQ1, we found 47,068 clone candidates between 111 Qualitas projects and 144,075 Stack Overflow snippets. They are 2,261 \textit{good} pairs, 26,838 \textit{ok} pairs. After classification, we found at least 523 online code clones pairs that are true clones between Stack Overflow and Qualitas.}
...
There were 28,832 online clone pair candidates for classifications as shown in \Cref{tab:online_clone_pairs}. We have use regular expressions to automatically classify \texttt{equals()} methods based on the method signature since they are not very interesting clones. It captured 26,102 clone pairs and classified them into pattern D. With help of the automatic classification, there were 3,450 pairs left for manual classification.

The first author who has been working on clone detection research took the role of a main investigator performing manual classification of all the 3,450 clone pairs. The second author took the role of a second investigator performing 10\% validation of the clone pairs (345 pairs) judged by the main investigator. By consulting the cloning patterns, the main investigator manually went through each agreed clone pair, looked at the clones, and chose the most appropriate cloning pattern for the pair. A relevant and useful observation was also recorded for each clone pair. The two investigators discussed and resolved 51 classification conflicts. There are three conflicts which the investigators could not find consensuses hence the third author was involved to solve the conflicts.
