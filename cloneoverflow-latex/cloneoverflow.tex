...
While a lot of research has focused on reusing code snippets ``from Stack Overflow'' (e.g.~\cite{Keivanloo2014,An2017,Yang2016}), less work has been conducted on finding the origins of code examples copied ``to Stack Overflow'' and whether these example snippets are still up-to-date.
...
\textbf{RQ2 (Patterns of online code clones):} \textit{Why do online code clones occur?} We categorise online clones into seven categories according to our online code cloning pattern scheme. This gives us more insights to why online code clones are created. %so we can differentiate and understand the motivation of cloning. 
...
\newline
\textbf{RQ3 (Outdated online code clones):} \textit{Are online code clones up-to-date compared to their counterparts in the original projects?} We are interested in the correctness of Stack Overflow code examples since millions of users are potentially reusing them.  %Is there observable problems caused by clones between Stack Overflow and open source projects? 
...
\textbf{RQ4 (Software licensing violation):} \textit{Do licensing conflicts occur between Stack Overflow clones and their originals?} We would like to investigate whether online code clones can be legally harmful to software reusing them.
...
\begin{table*}
	...
	\begin{tabular}{l|r|l|r|r|r|r}
		\hline
		Clone set & Original & Filter & Candidates & \texttt{equals()} & Manual & TP \\ 
		\hline 
		\multirow{1}{*}{\textit{good}} & --  & -- & 2,261  & 9 & 2,252 & 8 \\
		\multirow{1}{*}{\textit{ok}} & --  & -- & 26,838 & 9,122 & 200 & 47 \\
		%\multirow{1}{*}{\textit{agreed clone pairs}}        & 29,099 \\
		\multirow{1}{*}{$S_D$} & 67,570 & $Line \geq 10$, RegEx, good/ok & 17,801 & 16,930 & 871 & 432 \\
		\multirow{1}{*}{$N_D$} & 229,176 & $Sim \geq 80\%$, RegEx, good/ok & 168 & 41 & 127 & 36 \\ 
		%\multirow{1}{*}{\textit{disagreed clone pairs}}    & 1,039 \\
		\hline
		Total  & 296,746 & & 47,068 & 26,102 & 3,450 & 523 \\ 
		\hline
	\end{tabular} 
\end{table*}
...
Agreed clone pairs are clone pairs that pass Bellon's good- or ok-match criteria and selected for manual investigation.  Similar to the original study, we select a threshold \textit{p} of 0.7 for both \textit{good} and \textit{ok} pairs \cite{Bellon2007}. The number of projects processed by the clone detection tools are listed in \Cref{tab:projects_missing}. We found that NiCad generates errors and stopped processing against some Qualitas projects. \FIXME{Reported the errors to NiCad creator. Add response here.}  $N_D$ could not detect clones in 6 projects due to clustering errors. $N_E$ generated renaming and clustering errors for 11 and 16 projects respectively. As a result, we did not have NiCad clones from these project in the agreed clone pairs. Simian clones of the same projects also disappeared from the agreed clone pairs since they could not find matches.
