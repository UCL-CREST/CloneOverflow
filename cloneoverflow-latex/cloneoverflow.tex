...
\textbf{RQ2 (reasons for creating online code clones):} \textit{Why do online code clones occur?} We categorise online clones into seven groups according to our classification scheme. %so we can differentiate and understand the motivation of cloning. 
...
\begin{table}
	\centering
	\caption{The number of online clone pair condidates (mutually exclusive) after using clone agreement}
	...
	\begin{tabular}{l|r}
		...
		\hline 
		\multirow{1}{*}{\textit{good-match(0.7)}}  & 2,261  \\
		\multirow{1}{*}{\textit{ok-match(0.7)}}    & 29,099 \\
		\multirow{1}{*}{\textit{Simian$_{df}$}}    & 871    \\
		\multirow{1}{*}{\textit{NiCad$_{df}$}}     & 168    \\ 
		\hline
		Total                                      & 32,399 \\ 
		\hline
	\end{tabular} 
...
\begin{table}[H]
	...
	\begin{tabular}{l|l|r|r}
		...
		\hline
		\textit{df}     & \textit{df}       & 10 	     & 9,062     \\ 
		\textit{df}     & \textit{EvCl}     & 26 	     & 1017     \\ 
		\textit{EvCl} 	& \textit{df} 	    & 10 	     & 88       \\
		\textit{EvCl} 	& \textit{EvCl}     & 2,228      & 19,777    \\ 
		\hline
		\multicolumn{2}{l|}{Total}          & 2,274      & 29,944    \\
		\hline
		\multicolumn{2}{l|}{Total (unique)} & 2,261      & 29,099    \\
		\hline
	\end{tabular}
\end{table}
...
\begin{table}
	...
	\begin{tabular}{l|p{2.4cm}|r|r|r}
		...
		\hline
		\multirow{1}{*}{\textit{Simian$_{df}$}} & $minline \geq 10$ \newline reg.~expressions & 67,570 & 2,546 & 871 \\ %140 + 1482 + 926
		\hline
%		\multirow{1}{*}{\textit{NiCad$_{df}$}-1} & 7040  & 226 & 6700 & 114 \\
%		\multirow{1}{*}{\textit{NiCad$_{df}$}-2} & 22187  & 152 & 21990 & 45 \\
		\multirow{1}{*}{\textit{NiCad$_{df}$}} & $similarity \geq 80\%$ & 229,176  & 450 & 168 \\
		\hline
	\end{tabular} %
	}
\end{table}

\subsection{RQ2: Reasons for Creating Online Code Clones}
...
\begin{table*}
	...
	\resizebox{2.1\columnwidth}{!}{%
		\begin{tabular}{|l"r|r|r|r|r|r|r|r"r|r|r|r|r|r|r"r|r|r|r|}
			...
			\hline 
			\hline 
			\multirow{1}{*}{\textit{Simian$_{df}$}} & 79 & 0 & 336 & 17 & \textbf{432} & 148 & 229 & 38 & 209 & 63 & 167 & \textbf{439} & 136 & 214 & 49 & \textbf{871} & 271 & 428 & 58 \\
			\hline
...
Stack Overflow is a gold mine for software engineering research. Its rich and developer-driven data are valuable for software development. The data set has been put to use in several previous studies. In terms of developer-assisting tools, Seahawk is an eclipse plug-in that can search and recommend relevant code snippets from Stack Overflow \cite{Ponzanelli2013}. A follow up work, Prompter, by Ponzanelli et al.~\cite{Ponzanelli2014} is also an Eclipse plug-in for recommending relevant discussion from Stack Overflow but with improved algorithms. Several code search systems are proposed using whole or partial data from Stack Overflow \cite{Diamantopoulos2015,Keivanloo2014,Park2014, Stolee2014}. Treude et al.~try to do something \cite{Treude2016}.

Another aspect is knowledge extraction from Stack Overflow. Nasehi et al.~studied what makes a good code example by analysing answers from Stack Overflow \cite{Nasehi2012}. Studies to understand developers' behaviours on Stack Overlow include \cite{Wang2013_StackOverflow, Movshovitz-Attias2013,Rosen2016,Subramanian2013}.
A study of copy and paste behaviour has been done by Ahmed et al.~\cite{Ahmed2015}. Some studies aim to improve Stack Overflow systems \cite{Diamantopoulos2015, Wang2014, Bosu2013}.

Code clone detection is a long-standing research topic in software engineering. Whether clones are good or bad for software is still controversial \cite{Sajnani2016,Kapser2003,Kapser2008,Krinke2008,Hotta2010,Gode2011,Harder2013}. However, by only knowing how many code clones residing in software and how they evolve \cite{Pate2013,Mondal2011} can provide several insights into the systems. Clone detection has its several applications such as software plagiarism detection~\cite{Prechelt2002}, 
source code provenance~\cite{Davies2013}, and software licensing conflicts~\cite{German2009}.

Two code fragments are clones if they are similar similar enough according to a given definition of similarity \cite{Bellon2007}. Given an open interpretation of ``definition of similarity'', there are various clone detection tools and their siblings, code plagiarism detectors, invented. Some tools
use string comparison techniques such as Simian \cite{simian}. NiCad \cite{Roy2008,Cordy} also exploits Longest Common Subsequence (LCS) to discover clones after applying code pretty-printing using TXL \cite{Cordy2006}. Many tools do not work on original source code directly but transform them into an
...
To find more challenging clones such as type-3 clones, structural similarity of clones is needed.
...
Kapser et al.~studied clones in Linux file system subsystems and create patterns of code cloning: Forking, Templating, Customization and Exact match~\cite{Kapser2003,Kapser2008}. Our study partially adapted the online code clone classification scheme from their patterns.

There are a few studies comparing different clone detection techniques on various data sets \cite{Roy2008, Ragkhitwetsagul2016,Svajlenko2014}. Since clone detectors are different in their detection approaches, they behave differently and report different clones. Some researchers exploit different behaviours of clone detectors by finding agreement from them to obtain highly-confident clones \cite{Bellon2007}. Using the same data set, clone pairs that are agreed by several tools (or several code similarity measurement techniques) are highly possible to be true clones than the ones reported by only a single tool \cite{Wang2013,Ragkhitwetsagul2016_ssbse,Funaro2010}. These studies also report that sensitivities of the tools' parameter settings have strong effects to the results \cite{Ragkhitwetsagul2016}.

Software licensing is crucial for open source and industrial software development. Di Penta et al.~study an evolution of software licensing in six FOSS and found that licensing statements change over time \cite{DiPenta2010}. German et al.~performed an empirical study of code siblings, code clones among different systems coming from the same source, using CCFinderX clone detector. They found that licensing conflicts can occur between clone siblings. Ninka is a tool to automate identification of software license~\cite{German2010} from program source code. We use Ninka to analyse software license from Stack Overflow fragments and Qualitas projects. 

Reusing of third-party source code is very normal in software development. Xia et al.~\cite{Xia2014} show that a large number of open source systems reuse outdated third-party libraries of famous open source projects. Using outdated code give detrimental effects since they introduce vulnerability into the software. Our study discovers similar findings but in the context of reusing outdated code from Stack Overflow.
