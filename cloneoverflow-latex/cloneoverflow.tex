...
\begin{abstract}
  ...
  curated Qualitas corpus. We discovered and investigated 31,946
...
\subsection{Clone Detection Configuration}
...
As shown in \Cref{t_agreed_good_clone_pairs}, after we applied clone
agreement filtering using Bellon's criteria\footnote{Similar to the
  original study, we selected a threshold \textit{t} of 0.7 for both
  \textit{good} and \textit{ok} pairs~\cite{Bellon2007}.}, the number
of \textit{good} clone pairs is 2,308 and the number of \textit{ok}
clone pairs is 31,729, 0.1\textperthousand~of all reported clone
...
\Cref{t_agreed_good_clone_pairs}.  There are 2,321 (2,308 unique)
\textit{good} pairs consisting of 18 pairs from $S_D$--$N_D$, 26 pairs
from $S_D$--$N_E$, 10 pairs from $S_E$--$N_D$, and 2,267 pairs from
$S_E$--$N_E$. There are 32,560 (31,729 unique) \textit{ok}
pairs\footnote{This excludes the subsumed 2,308 \textit{good}
  pairs. According to the definition, \textit{ok} pairs always subsume
  \textit{good} pairs.}. We obtained 11,467; 993; 79; and 20,021 pairs
...
size, irrelevant methods, similarity threshold, and good/ok pairs. For
...
tried varying the UPI from 70\% (default, 632,855 pairs), 80\%
...
\subsubsection{Manual Inspection Data Set}

For the manual inspection of clone pairs, we selected all
\textit{good} pairs (2,308) and the \textit{ok} pairs for
$S_D$--$N_D$, $S_E$--$N_D$, and $S_D$--$N_E$ (11,749), but we ignored
...
Together with the 17,801 (referred as $S_D'$ set) and 88 (referred as
$N_D'$ set) clone pairs after filtering clone pairs from $S_D$ and
$N_D$, we ended up with a set of 31,946 online clone pair candidates
...
\subsection{The Seven Patterns of Online Code Cloning}

As we are interested in the patterns of online code clones, we studied
the 8 patterns of cloning from Kapser et
al.~\cite{Kapser2006,Kapser2008} and performed a preliminary study to
evaluate its applicability to our study. Using the $S_D$ clone report,
...
associated with 697 clone pairs. Using Kapser's cloning patterns, the
clone pairs were categorised into either Customisation or Templating.
However, Kapser's cloning patterns are too broad for our study and a
...
\emph{boiler-plate code} and \emph{API/library protocol}, and defined
...
The classification of all 31,946 online clone pairs followed the steps
...
\textbf{EX}. Lastly, if there is no evidence of copying in any
...
code cloning. For RQ3, we looked at the true positive clone pairs and
checked if they have been changed since they have been
cloned. Similarly, for RQ4, we looked at the license of each clone and
checked for a possibility of a licensing violation.
...
\begin{tabular}{l|rrrr}
  ...
  \hline 
  \multicolumn{4}{l}{\textit{Reported Clones}} \\
  \hline
  $S_D$ & 67,570 & 1,086 & 92 & 38\% \\
  $S_E$ & 63,635,844 & 1,531 & 111 & 50\% \\
  $N_D$ & 632,855& 1,392 & 82 & 30\% \\
  $N_E$ & 251,449,849& 12,886 & 88 & 33\% \\
  \hline
  Total & 315,786,118 & &\\
  \hline
  \multicolumn{4}{l}{\textit{Filtered Data Sets}} \\
  \hline
  \textit{good} & 2,308 & 92 & 66 & 23\% \\
  \textit{ok'} & 11,749 & 136 & 39 & 28\% \\
  $S_D'$ & 17,081 & 471 & 76 & 31\% \\
  $N_D'$ & 88 & 77 & 29 & 26\% \\
  \hline
  Total & 31,946 & 621 & 95 & 27\%\\
  \hline 
  \multicolumn{4}{l}{\textit{True Positives without AC}} \\
  \hline 
  $\textit{good}/\textrm{AC}$ & 136 & 47 & 25 & 23\% \\
  $\textit{ok'}/\textrm{AC}$ & 11,644 & 125 & 34 & 28\% \\
  $S_D'/\textrm{AC}$ & 17,673 & 448 & 72 & 31\% \\
  $N_D'/\textrm{AC}$ & 75 & 44 & 17 & 28\% \\
  \hline
  Total & 29,528 & 555 & 81 & 28\% \\
  \hline
\end{tabular}
\end{table}
...
combinations. As shown in \Cref{tab:snippets}, for the 2,308
\textit{good} pairs, there are 92 Stack Overflow snippets which are
associated with 66 Qualitas projects. The 11,749 \textit{ok'} pairs
are clones between 136 Stack Overflow snippets and 39 Qualitas
projects.  Moreover, for the disagreed pairs, we found 17,081 clone
pairs reported by $S_D'$ are clones between 471 Stack Overflow
snippets and 76 Qualitas projects and 88 $N_D'$ clone pairs are clones
between 77 Stack Overflow snippets and 29 Qualitas projects. In total
we found that the filtered data sets contains 31,946 clone pairs which
are clones between 621 Stack Overflow snippets and 95 Qualitas
projects. During the manual investigation, we identified 2,418 pairs
as being accidential clones (AC), i.e.~false positives. After removing
them, the data set still contains 29,528 clone pairs between 555 Stack
Overflow snippets and 81 Qualitas projects.

\textbf{To answer RQ1, we found most Qualitas projects to share
  similar code with Stack Overflow code snippets. Depending on the
  data set, between 82 to all 111 projects share similar code. The
  filtered data set has 95 projects share code with Stack
  Overflow. Even in the manually confirmed data set of 29,528 clone
  pairs, a majority of 81 Qualitas projects share similar code with
  Stack Overflow.}
...
We rely on a manual study to qualitatively investigate the patterns of
online code cloning. To achieve this, we classified all 31,946
selected clone pairs according to the process shown in
\Cref{fig:classification_process}.  Before performing the manual
classification, we created and used an automatic classifier 
to classify {\small\texttt{equals()}}, 
{\small\texttt{hashCode()}} and getters/setters. As shown in
\Cref{tab:online_clone_classification_results}, the automatic
classifier helped us to classify 28,314 clone pairs into
pattern \textbf{BP}.

There were 3,632 pairs left for the manual check. The first author,
who has been working on clone detection research for two years, took
the role of the main investigator performing a manual classification
of the 3,632 clone pairs. By consulting the cloning patterns, the main
investigator went through each clone pair candidate, looked at the
clones, and chose the most appropriate cloning pattern for the pair.
%
To mitigate the human errors of the manual classification, the second
author took the role of the validating investigator, performing a
validation on 10\% (363 pairs) of the clone pairs judged by the main
investigator. After the validation, the classifications from the two
investigators were compared and 64 conflicts were found. The two
investigators discussed and resolved 61 of them while the 3 conflicts
were resolved by involving the third author. The classification
results are shown in \Cref{tab:classification_good_o} and explained in
the following.
...
is ``copied'' or ``modified'' from a specific file or class in a
Qualitas project. The most cloned projects are \textsf{hibernate} and
...
any clone pairs with evidence of cloning from Stack Overflow to
Qualitas projects.

\textbf{EX: External Sources.} We found 197 online clone pairs with
evidence of cloning from external sources to Stack Overflow. They
include 4, 41, 146, and 6 clone pairs from the \textit{good},
\textit{ok'}, $S_D'$, and $N_D'$ set respectively.

These findings complement a study of clones between software
projects~\cite{Svajlenko2014}. We found that the code cloning can also
...
websites. For example, we found a clone of code snippet to convert
numbers into words which is copied from
\url{www.rgagnon.com/javadetails/java-0426.html}. Both the Stack
Overflow and the \textsf{compiere} project source code contain an
attribution to the original source. Another example is a code snippet
to generate graphical \textit{Perlin noise}. It is copied from
\url{http://mrl.nyu.edu/~perlin/noise/}. The code is used on Stack
...
There are other EX clone pairs which are copied from third-party
libraries such as \textsf{zxing}, \textsf{jasper}, \textsf{java.util},
\textsf{javax.servlet}, and the \textsf{bouncycastle} cryptography
project.

\textbf{UD: Unknown Direction.} We found 382 online clone pairs with
no evidence of cloning between Qualitas and Stack Overflow but with a
high code similarity that suggests cloning. They include 11, 42, 294,
and 35 pairs from the \textit{good}, \textit{ok'}, $S_D'$, and $N_D'$
set respectively. The most cloned projects is \textsf{netbeans} with
95 clone pairs. Most of the clones are a large chunk of code handling
...
boiler-plate code with filters before the analysis, we still observed
28,749 such clone pairs. The \textbf{BP} clone pairs account for
89.7\% of all clone pairs we classified. The majority of them are
{\small{\texttt{equals()}}} methods.

\textbf{IN: Inheritance/interface.} There were 111 clone pairs found
...
\textbf{AC: Accidental Clones.} There were 2,418 accidental clone
...
\begin{table}
	...
	%\resizebox{\columnwidth}{!}{%
		\begin{tabular}{l|rrrr|rrr|r}
			...
			\hline
			\multirow{1}{*}{\textit{good}}  & 3 & 0 & 4 & 11 & 111  & 7 & 2,172 & 2,308 \\
			\multirow{1}{*}{\textit{ok'}}  & 14 & 0 & 41 & 20 & 11,547 & 22 & 105 & 11,749 \\
			\multirow{1}{*}{$S_D'$} & 86 & 0 & 146 & 293 & 17,074 & 74 & 128 & 17,801 \\
			\multirow{1}{*}{$N_D'$} & 9  & 0  & 6 & 35 & 17 & 8 & 13 & 88 \\ 
			\hline
			Total & 112 & 0 & 197 & 359 & 28,749 & 111 & 2,418 & 31,946 \\ 
			\hline
		\end{tabular} 
	%}
\end{table}
...
\textbf{To answer RQ2, we found that a large amount of the clone pairs
  ...
  (28,749) or accidental clones (2,418) with no evidence that the code
  ...
  197 pairs were found to be cloned to Stack Overflow from external
...
clone from Qualitas a a proxy. We searched for its latest version by
...
snippets. %there are studies of automatic technique to identify software licensing from source code files~\cite{German2010}
...
\begin{table}[]
	...
	\begin{tabular}{l|p{1.95cm}l|rrr}
		...
		\hline
		\multicolumn{3}{l|}{Total} &  79 & 76 & 202 \\
		\hline
	\end{tabular} %
%}
\end{table}
...
\textbf{QS}, \textbf{EX}, and \textbf{UD} set. The summary of
licensing information is listed in \Cref{tab:license_abc}. The
...
license can be found. The table shows the license mapping of Stack
Overflow snippets and Qualitas source code. The results are reported
separately for the \textbf{QS}, \textbf{EX}, and \textbf{UD} set. In
overall, we can see that most of the Stack Overflow snippets do not
contain licensing terms while their clone counterparts in Qualitas
projects do.

\textbf{Compatible license:} There are 37 pairs which have compatible
...
code and text on Stack Overflow posted before 1st March 2016 are
protected by \emph{CC BY-NC-SA 3.0} (and \emph{MIT} license from 1st
March 2016 onwards), we can treat the Stack Overflow code snippets
...
\textbf{Incompatible license:} there are 357 clone pairs which do not
...
original source code from the external sources. We found that 76 out
of 87 EX clone pairs have incompatible licenses.
...
\textbf{For RQ4, we found 357 code snippets on Stack Overflow that
...
pairs we classified.  First, we used two, instead of one, widely-used
clone detection tools, Simian and NiCad.  We had tried three other
clone detectors but could not add them to the study due to their
scalability issue and susceptibility to incomplete code snippets.
Second, we adopted Bellon's clone agreement metric~\cite{Bellon2007}
to prioritise and filters clone pairs for the manual
classification. The agreed clone pairs that passed \textit{good} and
\textit{ok} criteria had a higher chance to be true positive. We also
...
detectors and their configurations.  We mitigated the problem of clone
detector's parameter sensitivity by adding another configuration,
besides the default configuration. We reused the established
configurations from the EvaClone study~\cite{Wang2013}.  We also
...
sampling.  The 64 classification conflicts were discussed and
resolved.  The third author made a final judgement when no consensus
...
extracted Java accepted answers. Instead of analysing every post, we
only focused on posts marked as accepted answers since they are the
most likely ones to be reused. This yields a total of 144,064
snippets.  Our findings are limited to these restrictions. They may
not be generalised to all programming languages and all answers on
Stack Overflow. On the other hand, we chose the curated Qualitas
...
classify 31,946 clone pairs using our seven patterns of online code
