...
\begin{table}
	...
	\begin{tabular}{l|r|c|c|c|c|r}
		...
		& & \textit{L} & \textit{S} & \textit{M} & \textit{G} & \\
		\hline 
		...
		\multirow{1}{*}{$N_D$} & 632,855 & \checkmark & \checkmark & \checkmark & \checkmark & 88 \\ 
		%\multirow{1}{*}{\textit{disagreed clone pairs}}    & 1,039 \\
%		\hline
%		Total  & 6,400,425 & \multicolumn{4}{c|}{ } & 17,169 \\ 
		\hline
		\multicolumn{7}{l}{*\textit{L}=size $\geq 10$, \textit{S}=sim $\geq 85\%$, \textit{M}=irr.~methods, \textit{G}=\textit{good}/\textit{ok} pairs.}  \\
	\end{tabular} %
	%}
\end{table}
...
for $S_D$ and 632,855 pairs for $N_D$). We applied four filters: clone
...
The \emph{irrelevant methods filter} removes clones of
\verb|hashCode()| methods and getters/setters which are almost always
false clones. The third filter, \emph{similarity threshold}, applies
...
results using clone size, irrelevant methods, and gook/ok pairs. The
three filters reduced the number of $S_D$ clones to 17,801 pairs.  We
...
\FIXME{Can you say something about the numbers only looking at the set
  of pairs used for manual inspection?}
% FIXME
% However, as it is well known that the precision of clone detectors is
c% not very good, it is clear that the above numbers may not be
...
\FIXME{Why equals here but not hashcode? Why isn't it the same
  irrelevant methods filter?}
...
classification, we used the irrelevant method filter to automatically
classify {\small\texttt{equals()}} and the remaining
getters/setters. As shown in
\Cref{tab:online_clone_classification_results}, the automatic
classifier helped us classified and captured 28,314 clone pairs into
pattern \textbf{BP}.
...
To answer RQ2, the classification results are explained and discussed
below.
...
\begin{table}
	\centering
	\caption{Classifications of online clone pairs. FIXME switch
          UD and EX}
...
\textbf{To answer RQ2, we found that a large amount of the clone pairs
  ...
  (28,642) or accidental clones (2,525) with no evidence that the code
  has actually been copied. Nevertheless, we found 110 pairs that had
  strong evidences to be cloned from Qualitas to Stack Overflow and
  178 pairs were found to be cloned to Stack Overflow from external
...
\FIXME{You are using the complete number of pairs in a project instead
  of the outdated pairs!!!}
...
shown in \Cref{fig:outdated}, \textsf{spring} and \textsf{hibernate}
have the highest number of 20 outdated pairs, followed by 15 from
\textsf{eclipse}, and 12 from \textsf{hadoop}. Besides the example of
outdated code in the \textsf{hadoop}'s
{\small{\texttt{WritableComparator.java}}} shown in
...
\begin{table}[]
	\centering
	\caption{License mapping of the online clones (file-level) in
          QS, UD, and EX FIXME switch UD and EX}
	\label{tab:license_abc}
	\small
	%\resizebox{\columnwidth}{!}{%
	\begin{tabular}{l|p{2cm}l|rrr}
		\hline
...
the Qualitas projects came with a software license (shown in
\Cref{t:q_projects_license}), we found that the licensing information
