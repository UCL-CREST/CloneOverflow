...
To answer the four research questions, we derived three result sets; 
\textit{Reported Clones}, \textit{Filtered Clones}, and \textit{Pattern Clones};
and a manual investigation of online clones. 
For the Reported Clones set, we deployed code clone detection
to locate similar code,~i.e.~clones, between Stack Overflow and 
Qualitas projects. The second result set, Filtered Clones, is derived from the
Reported Clones set by filtering out trivial clones. The last result set,
Pattern Clones, is used to define the online code cloning patterns
for our manual classification. We show the number of clone pairs in the Reported Clones,
Filtered Clones and Pattern Clone result set in \Cref{tab:result_sets}.

\subsection{RQ1: Online Code Clones}

With the Reported Clones and Filtered Clones result sets, 
we can observe how many code snippets on
Stack Overflow are reported as clones to/from Qualitas projects
in two aspects: before and after removing trivial clones. 
We explain the approach we used to create to two result sets below.

\begin{table}
	\centering
	\caption{Clone Result Sets}
	\label{tab:result_sets}
	%\small
	\begin{tabular}{lp{3cm}r}
		\hline 
		Result set & Description & Clone pairs \\
		\hline
		Reported Clones & All reported clones & 315,786,118 \\ 
		%\hline 
		Filtered Clones & Clones after filtering & 32,533 \\ 
		Pattern Clones & Clones for defining online cloning patterns & 697 \\
		\hline 
	\end{tabular} 
\end{table}

\subsubsection{Reported Clones Result Set} 
...
datasets. Due to a scalability issue, we partitioned the input and ran the tools multiple
...
\subsubsection{Filtered Clones Result Set}
...
\begin{figure}
	\centering
	\includegraphics[width=\linewidth]{good-ok_pairs}
...
%\subsubsection{Filtered Clones Set}
...
evaluate its applicability to our study by using a small subset of the
reported online clones. 

\subsubsection{Pattern Clones Result Set}
...
\subsubsection{Defining Online Code Cloning Patterns}
Using Kapser's cloning patterns and the Pattern Clones Result Set, the
...
classification of the filtered clone pairs. By consulting the cloning 
patterns, the main investigator went through each clone pair candidate, 
looked at the clones, and chose the most appropriate cloning pattern 
for the pair. To mitigate the human errors of the manual classification, 
...
If the two investigator cannot find a consensus, the third author is 
...
projects. During the manual investigation, we identified 2,420 pairs
as being accidential clones (AC), i.e.~false positives. After removing
them, the data set still contains 30,113 clone pairs between 556 Stack
Overflow snippets and 80 Qualitas projects.
...
\textbf{To answer RQ1, we found most Qualitas projects to share
  ...
  data set, between 88 to all 111 projects share similar code. The
  filtered data set has 96 projects share code with Stack
  Overflow. Even in the manually confirmed data set of 30,113 clone
  pairs, a majority of 80 Qualitas projects share similar code with
  Stack Overflow.}
...
is ``copied'', ``borrow'' or ``modified'' from a specific file or class in a
...
evidence of cloning from external sources to Stack Overflow. 
They include 4, 44, 146, and 6 clone pairs from the \textit{good},
\textit{ok'}, $S_D'$, and $N_D'$ set respectively. All of them
contain statements showing an external source of the cloned code.
For example, Stack Overflow Post 9549009 contains a code comment saying \textit{``Copied 
	shamelessly from org.bouncycastle.crypto.generators.PKCS5S2ParametersGenerator,
