...
\vspace{0.5cm} \noindent\fbox{%
	\parbox[c][2cm]{0.98\textwidth}{%
		\textit{For RQ 1, we found that answering questions by writing the new code from
			scratch is the most popular choice for Stack Overflow answerers followed by modifying from the code in question, or copying from personal projects. Other less
			popular choices include copying code from open
			source projects and from other sources. Copying code from company projects are the least popular choice.} }%
}
\vspace{0.5cm}
...
\vspace{0.5cm} \noindent\fbox{%
	\parbox[c][1.5cm]{0.98\textwidth}{%
		\textit{For RQ4, Stack Overflow is ranked higher than official documentations, online repositories, and book as the resource to look for programming solutions. 
		Developers rely on Stack Overflow answers because they are easy to search for on the web. Moreover, 64\% of the participants reuse code snippets from
	Stack Overflow at least once a week.}
	}%
} \vspace{0.5cm}
